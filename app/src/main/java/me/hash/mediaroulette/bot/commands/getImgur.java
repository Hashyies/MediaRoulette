...
public class getImgur extends ListenerAdapter {
    @Override
    public void onSlashCommandInteraction(SlashCommandInteractionEvent event) {
        if (!event.getName().equals("random-imgur"))
            return;

        event.deferReply().queue();
        Bot.executor.execute(() -> {
            if (!Bot.config.getOrDefault("IMGUR", true, Boolean.class)) {
            ...
            Embeds.sendImageEmbed(event, "Here is your random Imgur Image:", RandomImage.getImgurImage(), false);
        });
    }
}
